...
const program = scrapeIt("https://www.neos.io/events/neos-conference-hamburg-2018/program.html", {
    days: {
        listItem: "table",
        data: {
            topics: {
...
const speakersWithTalkDetails = scrapeIt("https://www.neos.io/events/neos-conference-hamburg-2018/speakers.html", {
    speakers: {
            ...
            talks: {
                ...
                data: {
                    ...
                    description: {
                        closest: '',
                        how: (h2Node) => {
                            return h2Node.closest('.neos-nodetypes-headline').next().filter('.neos-nodetypes-text').text();
                        },
                    }
                }
            }
...
const isBreakOrLunch = title => (title.indexOf('Break') !== -1 || title === 'Lunch');
...
const transformTopic = (topicFromCrawler, topicTitle, roomName, speakersWithTalkDetails) => ({
...
Promise.all([program, speakersWithTalkDetails])
    .then(([program, speakersWithTalkDetails]) => {
        ...
        const transformedDays = program.days.map(day => {
            let transformedTopics = [];
            day.topics
                // remove header rows
                .filter(topicFromCrawler => topicFromCrawler.time && topicFromCrawler.time !== 'Time')
                .forEach(topicFromCrawler => {
                    if (topicFromCrawler.topicRoom1) {
                        transformedTopics.push(transformTopic(topicFromCrawler, topicFromCrawler.topicRoom1, 'Room 1', speakersWithTalkDetails));
                    }
                    if (topicFromCrawler.topicRoom2) {
                        transformedTopics.push(transformTopic(topicFromCrawler, topicFromCrawler.topicRoom2, 'Room 2', speakersWithTalkDetails));
                    }
                });
    ...
    .then(transformedTopics => {
        fs.writeFileSync(__dirname + '/../app/data/talks.json', JSON.stringify(transformedTopics));
    });